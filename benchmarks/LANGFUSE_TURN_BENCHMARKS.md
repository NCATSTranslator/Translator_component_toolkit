...
## Optional Codex development setup

The helper below reads a trusted shell environment file (default: `.env`)
and writes local `.codex/langfuse.json` with mode 600. It requires
...
This configures the developer's Codex integration, separately from TCT's
runtime opt-in flag. It is not required to import TCT or run the CLI/MCP server.
